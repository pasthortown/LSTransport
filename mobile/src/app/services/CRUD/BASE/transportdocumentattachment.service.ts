...
export class TransportDocumentAttachmentService {

   url = environment.api_base + 'transportdocumentattachment/';
   ...
   post(transportdocumentattachment: TransportDocumentAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(transportdocumentattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(transportdocumentattachment: TransportDocumentAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(transportdocumentattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
