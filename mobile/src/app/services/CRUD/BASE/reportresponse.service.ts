...
export class ReportResponseService {
   ...
   url = environment.api_base + 'reportresponse/';
   ...
   post(reportresponse: ReportResponse): Promise<any> {
      return this.http.post(this.url, JSON.stringify(reportresponse), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(reportresponse: ReportResponse): Promise<any> {
      return this.http.put(this.url, JSON.stringify(reportresponse), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
