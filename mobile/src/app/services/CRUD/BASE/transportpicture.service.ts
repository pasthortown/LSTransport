...
export class TransportPictureService {

   url = environment.api_base + 'transportpicture/';
   ...
   post(transportpicture: TransportPicture): Promise<any> {
      return this.http.post(this.url, JSON.stringify(transportpicture), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(transportpicture: TransportPicture): Promise<any> {
      return this.http.put(this.url, JSON.stringify(transportpicture), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
