...
export class RouteService {
   ...
   url = environment.api_base + 'route/';
   ...
   post(route: Route): Promise<any> {
      return this.http.post(this.url, JSON.stringify(route), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(route: Route): Promise<any> {
      return this.http.put(this.url, JSON.stringify(route), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
