...
export class PassengerBinacleService {

   url = environment.api_base + 'passengerbinacle/';
   ...
   post(passengerbinacle: PassengerBinacle): Promise<any> {
      return this.http.post(this.url, JSON.stringify(passengerbinacle), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(passengerbinacle: PassengerBinacle): Promise<any> {
      return this.http.put(this.url, JSON.stringify(passengerbinacle), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
