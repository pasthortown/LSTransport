...
export class LocationService {
   ...
   url = environment.api_base + 'location/';
   ...
   post(location: Location): Promise<any> {
      return this.http.post(this.url, JSON.stringify(location), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(location: Location): Promise<any> {
      return this.http.put(this.url, JSON.stringify(location), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
