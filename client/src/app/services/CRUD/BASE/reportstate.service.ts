...
export class ReportStateService {
   ...
   url = environment.api_base + 'reportstate/';
   ...
   post(reportstate: ReportState): Promise<any> {
      return this.http.post(this.url, JSON.stringify(reportstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(reportstate: ReportState): Promise<any> {
      return this.http.put(this.url, JSON.stringify(reportstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
