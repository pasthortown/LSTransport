...
export class PassengerService {
   ...
   url = environment.api_base + 'passenger/';
   ...
   post(passenger: Passenger): Promise<any> {
      return this.http.post(this.url, JSON.stringify(passenger), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(passenger: Passenger): Promise<any> {
      return this.http.put(this.url, JSON.stringify(passenger), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
