...
import { AccountRolAssigment } from './../../../models/BASE/AccountRolAssigment';

@Injectable({
   providedIn: 'root'
})
export class AccountRolAssigmentService {

   url = environment.api_base + 'accountrolassigment/';
   ...
   post(accountrolassigment: AccountRolAssigment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(accountrolassigment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(accountrolassigment: AccountRolAssigment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(accountrolassigment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
