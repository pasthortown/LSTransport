...
export class ReportService {
   ...
   url = environment.api_base + 'report/';
   ...
   post(report: Report): Promise<any> {
      return this.http.post(this.url, JSON.stringify(report), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(report: Report): Promise<any> {
      return this.http.put(this.url, JSON.stringify(report), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
