...
export class DriverService {
   ...
   url = environment.api_base + 'driver/';
   ...
   post(driver: Driver): Promise<any> {
      return this.http.post(this.url, JSON.stringify(driver), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(driver: Driver): Promise<any> {
      return this.http.put(this.url, JSON.stringify(driver), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
