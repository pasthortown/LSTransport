...
export class TransportService {
   ...
   url = environment.api_base + 'transport/';
   ...
   post(transport: Transport): Promise<any> {
      return this.http.post(this.url, JSON.stringify(transport), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(transport: Transport): Promise<any> {
      return this.http.put(this.url, JSON.stringify(transport), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
