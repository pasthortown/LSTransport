...
export class TransportDocumentService {
   ...
   url = environment.api_base + 'transportdocument/';
   ...
   post(transportdocument: TransportDocument): Promise<any> {
      return this.http.post(this.url, JSON.stringify(transportdocument), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(transportdocument: TransportDocument): Promise<any> {
      return this.http.put(this.url, JSON.stringify(transportdocument), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
